...
#factors from smaller to biggest
    biggest_factor = decomposition[-1] assert(
        biggest_factor[1] == 1) return biggest_factor[0]
...
#Curve parameter
          param = -0xd201000000010000
                  ...
                  prime_r = r(param) assert(
              prime_r ==
              52435875175126190479447740508185965837690552500527637822603658699938581184513)

              prime_q = q(param) print(prime_q)
                  assert(prime_q == 4002409555221667393417789825735904156556882819939007885332058136124031650490837864442687629129015664037894272559787) if (
                      mod(prime_q, 6) != 1)
...
#E / Fq
                          curve = EllipticCurve(Fq, [ 0, 4 ]) curve_order =
                              curve.order()

#Cofactors
                                  _h1 = curve_order // g1_order(curve_order)
    h1 = g1_h(param) assert(_h1 == h1) print('h1 = {}'.format(h1)) h2 =
        g2_h(param) print('h2 = {}'.format(h2))
