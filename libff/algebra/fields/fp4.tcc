...
{
    return Fp4_model<n,modulus>(this->coeffs[0] + other.coeffs[0],
...
{
    return Fp4_model<n,modulus>(lhs*rhs.coeffs[0],
                                lhs*rhs.coeffs[1]);
}
...
Fp4_model<n,modulus> Fp4_model<n,modulus>::mul_by_023(const Fp4_model<n,modulus> &other) const
{
    ...
    const my_Fp2 &B = other.coeffs[1], &A = other.coeffs[0],
        &b = this->coeffs[1], &a = this->coeffs[0];
    const my_Fp2 aA = my_Fp2(a.coeffs[0] * A.coeffs[0], a.coeffs[1] * A.coeffs[0]);
    const my_Fp2 bB = b*B;
...
{
    return Fp4_model<n,modulus>(coeffs[0].Frobenius_map(power),
