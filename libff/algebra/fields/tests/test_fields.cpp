...
do_test_fixed_wnaf(const FieldT &value, const size_t digit_size)
{
    ...
    //   n = num_digits,
    //   c = digit size and
    //   d_i = i-th fixed-wnaf digit of size c
    FieldT accum = FieldT::zero();
    for (size_t i = 0 ; i < num_digits ; ++i)
    {
        accum = accum * FieldT((long)1 << digit_size);
        ssize_t digit = fixed_wnaf_digit(v, digit_size, num_digits - 1 - i);
...
test_fixed_wnaf()
{
    ...
    {
        do_test_fixed_wnaf(FieldT(-1), i);
        do_test_fixed_wnaf(FieldT(-2), i);
    }
}
