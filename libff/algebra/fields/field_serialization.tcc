...
public:
    ...
    static void read(
        FieldT &field_el, const uint8_t buffer[field_binary_size<FieldT>()])
    {
        constexpr size_t base_field_size = field_binary_size<base_field_t>();
        for (size_t i = 0; i < FieldT::tower_extension_degree; ++i) {
            field_element_codec<encoding_binary, Form, base_field_t>::read(
                field_el.coeffs[i], buffer + (i * base_field_size));
        }
    }
    ...
    {
        constexpr size_t base_field_size = field_binary_size<base_field_t>();
        // For sub-buffers of size base_field_size, read the first component
        // with flags, then all remaining components as normal.
        field_element_codec<encoding_binary, Form, base_field_t>::
            read_with_flags(field_el.coeffs[0], flags, buffer);
        for (size_t i = 1; i < FieldT::tower_extension_degree; ++i) {
            field_element_codec<encoding_binary, Form, base_field_t>::read(
                field_el.coeffs[i], buffer + (i * base_field_size));
        }
...
{
protected:
    // Read a bigint value from a raw buffer.
    static void bigint_from_buffer(
        libff::bigint<n> &res, const uint8_t buffer[sizeof(libff::bigint<n>)])
    {
        // For now, assume that limbs are 64 bits wide (otherwise a warning
        // will be generated by __builtin_bswap64).
        const mp_limb_t *limbs_buffer = (const mp_limb_t *)buffer;
        for (size_t i = 0; i < n; ++i) {
            res.data[n - 1 - i] = __builtin_bswap64(limbs_buffer[i]);
        }
    }
...
public:
    ...
    static void read(
        Field &field_el, const uint8_t buffer[field_binary_size<Field>()])
    {
        // Read bigint from stream, reverse bytes in-place and convert to field
        // element.
        if (Form == form_plain) {
            libff::bigint<n> res;
            bigint_from_buffer(res, buffer);
            field_el = Field(res);
        } else {
            bigint_from_buffer(field_el.mont_repr, buffer);
        }
    }
    ...
    static void read_with_flags(
        Field &field_el,
        mp_limb_t &flags,
        const uint8_t buffer[field_binary_size<Field>()])
    {
        assert(verify_flag_capacity());
...
{
    internal::field_element_codec<encoding_binary, Form, FieldT>::
        read_with_flags(v, flags, in_s);
}
...
{
    internal::field_element_codec<encoding_binary, Form, FieldT>::
        read_with_flags(v, flags, buffer);
}
...
{
    internal::field_element_codec<encoding_binary, Form, FieldT>::
        write_with_flags(v, flags, out_s);
}
