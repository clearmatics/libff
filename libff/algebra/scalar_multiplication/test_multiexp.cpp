...
template<typename GroupT> void test_multiexp_accumulate_buckets(const size_t num_buckets)
{
    ...
    // Actual value
    const GroupT actual = multiexp_accumulate_buckets(
        values,
        value_hit,
        num_buckets);
...
template<typename GroupT, bool MixedAddition> void test_multiexp_signed_digits_round(const size_t digit_idx)
{
    using FieldT = typename GroupT::scalar_field;
    using BigIntT =
    ...
    // Actual value
    const GroupT actual = multiexp_signed_digits_round<GroupT, BigIntT, MixedAddition>(
        bases.begin(),
        bases.end(),
        exponents.begin(),
        buckets,
...
template<typename GroupT, multi_exp_method Method> void test_multiexp_inner()
{
    ...
    //   = [4] + [6] + [6] + 4
    //   = [20]
    const long expected_unencoded = 2 * (
            (4 << 0) + ((3 << c) * 2) + ((2 << (2 * c)) * 3) + ((1 << (3 * c)) * 4)
    );
